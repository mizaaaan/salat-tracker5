...
/** Card on the Tools menu screen */
const ToolCard = ({ icon, title, subtitle, onPress, disabled, styles }) => (
  <TouchableOpacity
  ...
  >
    ...
    {!disabled && <Text style={styles.toolChevron}>›</Text>}
  </TouchableOpacity>
);

/** Full streak detail view — same data/logic as before, now opened from the menu */
function StreakDetail({ onBack, styles, Colors }) {
    ...
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.weekCard}>
          {weekly.map((d, i) => (
            <DayCell key={i} dayName={d.dayName} completed={d.completed} allDone={d.allDone} styles={styles} />
          ))}
        </View>
      </View>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.miniGrid}>
          {TRACKABLE_PRAYERS.map((p) => (
            <View key={p} style={[styles.miniPill, todayData?.allDone && styles.miniPillDone]}>
              <Text style={[styles.miniPillText, todayData?.allDone && styles.miniPillTextDone]}>{p}</Text>
            </View>
          ))}
        </View>
      </View>
...
export default function ToolsScreen() {
  ...
  const [activeTool, setActiveTool] = useState(null); // null = menu

  if (activeTool === 'streak') {
    return (
      <SafeAreaView style={styles.container}>
        <StreakDetail onBack={() => setActiveTool(null)} styles={styles} Colors={Colors} />
      </SafeAreaView>
    );
  }
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        <Text style={styles.pageTitle}>🛠️ Tools</Text>
        <Text style={styles.pageSubtitle}>Extra features to support your worship</Text>
        ...
        <View style={styles.toolList}>
          <ToolCard
            ...
            onPress={() => setActiveTool('streak')}
            styles={styles}
          />
          <ToolCard icon="📿" title="Tasbih Counter"     subtitle="Coming soon" disabled styles={styles} />
          <ToolCard icon="🗓️" title="Islamic Calendar"   subtitle="Coming soon" disabled styles={styles} />
          <ToolCard icon="🕋" title="Hajj & Umrah Guide"  subtitle="Coming soon" disabled styles={styles} />
        </View>
...
const getStyles = (Colors) => StyleSheet.create({
  ...
  toolChevron: { fontSize: 22, color: Colors.textSecondary },

  // Back button (streak detail)
  ...
  bigNumber: { fontSize: 80, fontWeight: '900', color: Colors.primary, lineHeight: 88 },
  dayLabel: { fontSize: 18, fontWeight: '600', color: Colors.text, marginTop: 4 },
  streakMsg: { fontSize: 13, color: Colors.textSecondary, marginTop: 6, textAlign: 'center' },
  flameRow: { flexDirection: 'row', marginTop: 14, gap: 4 },
  flame: { fontSize: 20 },
  ...
  statLabel: {
    fontSize: 9, color: Colors.textSecondary, fontWeight: '600',
    letterSpacing: 0.8, textTransform: 'uppercase', marginTop: 4, textAlign: 'center',
  },
  ...
  miniPillDone: { borderColor: Colors.primary + '60', backgroundColor: Colors.primary + '15' },
  miniPillText: { color: Colors.textSecondary, fontSize: 13, fontWeight: '500' },
  ...
  quoteCard: {
    ...
    borderWidth: 1, borderColor: Colors.border, alignItems: 'flex-start', gap: 8,
  },
